...
export function ContainerModal({ open, onOpenChange, container, onSave, nextSNo }: ContainerModalProps) {
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      containerTypeId: formData.containerTypeId ? parseInt(formData.containerTypeId, 10) : null,
      containerTypeName: selectedContainerType?.name || null,
      sealNo: formData.actualSeal,
      noOfPcs: parseInt(formData.noOfPcs, 10) || 0,
      packageTypeId: formData.packageTypeId ? parseInt(formData.packageTypeId, 10) : null,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-modal-4xl bg-card border border-border p-0 max-h-[90vh] overflow-hidden flex flex-col gap-0">
